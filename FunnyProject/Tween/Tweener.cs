    ...
    public partial class Tween
    {
        public class TweenerImpl
        {
            ...
            private void AddAndRemove()
            {
                ...
                {
                    List<Tween> list;
                    if (add == null) continue; // was return before
                ...
                {
                    List<Tween> list;
                    if (remove == null) continue; // was return before
            ...
            private void ApplyBulkControl(object[] targets, Action<Tween> action)
            {
                foreach (var target in targets)
                {
                    ...
                    {
                    List<Tween> list;
                    if (target == null) continue; // was return before
                    if (_tweens.TryGetValue(target, out list))
                    {
                        foreach (var glide in list.ToList())
                        {
                            if (glide == null) continue;
                            action(glide);
                        }
                    }
                }catch
